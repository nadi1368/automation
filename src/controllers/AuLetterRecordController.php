...
use hesabro\automation\events\AuLetterRecordEvent;
 ...
 */
class AuLetterRecordController extends AuLetterController
{
            ...
            'access' => [
                ...
                'rules' =>
                    [
                        [
                            'allow' => true,
                            'roles' => ['automation/au-letter-record/view', 'superadmin', 'MBT_WORKER'],
                            'actions' => ['index', 'view', 'print']
                        ],
                        [
                            'allow' => true,
                            'roles' => ['automation/au-letter-record/action', 'superadmin', 'MBT_WORKER'],
    ...
    public function actionIndex()
    {
        ...
        $searchModel->type = AuLetter::TYPE_RECORD;
    ...
    {
        $model = new AuLetter(['type' => AuLetter::TYPE_RECORD, 'date' => Yii::$app->jdf::jdate("Y/m/d")]);
        $model->setScenario(AuLetter::SCENARIO_CREATE_RECORD);
        if ($this->request->isPost) {
            if ($model->load(Yii::$app->request->post()) && $model->validate()) {
                $transaction = Yii::$app->db->beginTransaction();
                $this->trigger(self::EVENT_BEFORE_CREATE, AuLetterRecordEvent::create($model));
                try {
                    $flag = $model->save(false);
                    if ($flag) {
                        $this->trigger(self::EVENT_AFTER_CREATE, AuLetterRecordEvent::create($model));
                        $transaction->commit();
    ...
    public function actionUpdate($id)
    {
        ...
        if ($this->request->isPost) {
            if ($model->load(Yii::$app->request->post()) && $model->validate()) {
                $transaction = Yii::$app->db->beginTransaction();
                $this->trigger(self::EVENT_BEFORE_UPDATE, AuLetterRecordEvent::create($model));
                try {
                    $flag = $model->save(false);
                    if ($flag) {
                        $this->trigger(self::EVENT_AFTER_UPDATE, AuLetterRecordEvent::create($model));
                        $transaction->commit();
                        $this->flash('success', Module::t('module', "Item Created"));
                        return $this->redirect(['view', 'id' => $model->id]);
                    } else {
